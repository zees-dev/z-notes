...
export type Reply =
  ...
  | {
      ...
      /** MEASURED against the live CLIProxyAPI gateway (gpt-5.6-sol): a real
...
export interface MockUpstream {
   ...
   * CLIProxyAPI advertises only none|low|medium|high). Distinct from
...
export async function startMockUpstream(): Promise<MockUpstream> {
  ...
  async function writeResponses(r: Reply, send: Send, rec: RecordedRequest) {
    ...
    /* `cache_write_tokens` is MEASURED from the live CLIProxyAPI gateway, which
