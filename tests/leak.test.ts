...
describe("leak canary — the AI relay (SPEC §11, phase 4)", () => {
  ...
  beforeAll(async () => {
    ...
    const put = await srv.api("PUT", "/api/settings", {
      ai: { baseUrl: mock.baseUrl, apiKey: "sk-leakgate-key", model: "gpt-5.6-sol", effort: "high" },
...
describe("leak canary — the terminal runner (SPEC §11, phase 5)", () => {
  ...
  beforeAll(async () => {
    ...
    const put = await tsrv.api("PUT", "/api/settings", {
      ai: { baseUrl: tmock.baseUrl, apiKey: "sk-termleak-key", model: "gpt-5.6-sol", effort: "high" },
      git: { autoSyncSeconds: 600 },
...
describe("leak canary — the vault is OPEN and every block is revealed (SPEC §11, phase 6)", () => {
  ...
  beforeAll(async () => {
    ...
    const put = await bsrv.api("PUT", "/api/settings", {
      ai: { baseUrl: mock.baseUrl, apiKey: "sk-revealgate-key", model: "gpt-5.6-sol", effort: "high" },
