...
beforeAll(async () => {
  ...
  const put = await srv.api("PUT", "/api/settings", {
    ai: { baseUrl: mock.baseUrl, apiKey: KEY, model: "gpt-5.6-sol", effort: "high" },
...
async function restoreSettings() {
  await srv.api("PUT", "/api/settings", {
    ...
    ai: { effort: "high", baseUrl: mock.baseUrl, model: "gpt-5.6-sol" },
  });
}
...
describe("settings × the AI statusbar item — the two surfaces cannot disagree", () => {
  ...
  test("changing the reasoning effort in Settings repaints the statusbar chip", async () => {
    ...
    expect(`chip before: ${(await aiChip()).txt}`).toBe("chip before: gpt-5.6-sol · high");
    ...
    await waitUntil(async () => (await aiChip()).txt === "gpt-5.6-sol · low", {
      timeout: 15000,
      interval: 150,
      label: "the chip to adopt the new effort",
    });
    expect(`chip after: ${(await aiChip()).txt}`).toBe("chip after: gpt-5.6-sol · low");
...
describe("routing × Settings — every way in is one navigation to /settings", () => {
  ...
  test("a hard reload of a deep URL re-applies the stored settings and re-paints the chip", async () => {
    ...
    await waitUntil(async () => (await aiChip()).txt === "gpt-5.6-sol · medium", {
      timeout: 25000,
      interval: 200,
      label: "the chip to paint the stored effort after a reload",
    });
    expect(`chip after reload: ${(await aiChip()).txt}`).toBe("chip after reload: gpt-5.6-sol · medium");
