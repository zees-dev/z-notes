...
**Pluggable API endpoint**: settings hold a base URL + API key + model name, speaking the OpenAI-compatible chat/completions protocol. Works with the user's local CLIProxyAPI (subscription-backed), OpenAI, or any compatible endpoint. Default model **`gpt-5.6-sol`** ("sol 5.6"), with configurable reasoning effort, **default high**. All of it configurable in the settings view.
