...
OpenAI-compatible endpoint (default `gpt-5.6-sol`, configurable reasoning effort, default
...
1. **Use `/v1/responses` as the primary wire protocol.** This is not a preference — for
   `gpt-5.6-*`, OpenAI *hard-rejects* function tools combined with any reasoning effort
...
2. **Keep the edit contract in a plain `type: "function"` tool**, not the native
   ...
   gateway on the path (CLIProxyAPI, LiteLLM, OpenRouter, vLLM) translates correctly.
...
`gpt-5.6-sol` (the `gpt-5.6` alias routes here) is the flagship reasoning model of the 5.6
...
| Default effort | `medium` (in **both** APIs, when omitted) | [Upgrading to gpt-5.6-sol](https://developers.openai.com/api/docs/guides/upgrading-to-gpt-5p6-sol.md) |
...
- [LiteLLM #33221](https://github.com/BerriAI/litellm/issues/33221) — filed 2026-07-14
  against LiteLLM v1.92.0; affects `gpt-5.6-sol`, `-luna`, `-terra`.
...
|---|---|---|---|
| **CLIProxyAPI** (the user's local proxy, `127.0.0.1:8317`) | Yes | **Yes** — `POST /v1/responses`, `POST /v1/responses/compact`, `GET /v1/responses` (WebSocket) | [API overview](https://router-for-me-cliproxyapi.mintlify.app/api/overview) |
...
CLIProxyAPI is the important one, and it is well-positioned: the Codex backend it fronts
...
([chat docs](https://router-for-me-cliproxyapi.mintlify.app/api/openai/chat)). Another
...
CLIProxyAPI translation is reliable for **flat `type: "function"` tools** and unreliable for
anything else:

- [CLIProxyAPI #3298](https://github.com/router-for-me/CLIProxyAPI/issues/3298): Codex-style
...
- Codex CLI's `apply_patch` is shipped as `"type": "custom"` with a **Lark grammar** rather
  than JSON parameters; CLIProxyAPI does not know how to translate that for non-OpenAI
  upstreams.
...
2. **CORS.** CLIProxyAPI happens to send `Access-Control-Allow-Origin: *`
   ([overview](https://router-for-me-cliproxyapi.mintlify.app/api/overview)), but arbitrary
...
CLIProxyAPI additionally offers a WebSocket transport at `ws://host/v1/responses` with
`previous_response_id` chaining
([streaming docs](https://router-for-me-cliproxyapi.mintlify.app/api/openai/streaming)).
...
|---|---|---|---|
| **A. Native `apply_patch` tool** | `{"type":"apply_patch"}` → typed `apply_patch_call` items with `operation: {type, path, diff}` | OpenAI + OpenRouter (renamed) only; Lark-grammar variant breaks CLIProxyAPI translation | Lowest for OpenAI models (trained on it) |
...
Read the GPT-5.4 column — it is the closest available proxy for `gpt-5.6-sol`. Two things
stand out:
...
1. **Not portable.** Vendor-namespaced at OpenRouter (`openrouter:apply_patch`), absent from
   local servers, and the Codex CLI's Lark-grammar spelling is a known CLIProxyAPI
...
Z-Notes-Proposal: <id>
Z-Notes-Model: gpt-5.6-sol@high
```
...
```jsonc
ai: {
  baseUrl:  "http://127.0.0.1:8317/v1",   // CLIProxyAPI default
...
- [CLIProxyAPI — API overview](https://router-for-me-cliproxyapi.mintlify.app/api/overview)
- [CLIProxyAPI — Chat Completions](https://router-for-me-cliproxyapi.mintlify.app/api/openai/chat)
- [CLIProxyAPI — Streaming & WebSockets](https://router-for-me-cliproxyapi.mintlify.app/api/openai/streaming)
- [CLIProxyAPI #3298 — namespace tools not flattened](https://github.com/router-for-me/CLIProxyAPI/issues/3298)
- [CLIProxyAPI #1709 — /v1/responses vs /v1/chat/completions](https://github.com/router-for-me/CLIProxyAPI/issues/1709)
- [LiteLLM — /responses endpoint](https://docs.litellm.ai/docs/response_api)
- [LiteLLM #33221 — function tools + reasoning_effort 400 on gpt-5.6](https://github.com/BerriAI/litellm/issues/33221)
